...
5. Запусти: `python -m english_bot.bot`
...
- Логи: `logs/bot.log` (ротация, 1 МБ × 3).

## Как это работает

- `english_bot/bot.py` — Telegram-логика (сообщения, реакции, ответы).
- `english_bot/ai.py` — вызов Gemini: `ai.evaluate() -> dict` (уровень, ошибки, исправление, объяснение, перевод).
- `english_bot/exceptions.py` — кастомные исключения (`ConfigError`, `AIError`).
